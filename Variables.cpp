...
void Variables::changeVariablesInExpression(std::string& exp) {
    ...
    std::string token;
    std::set<std::string> operationsAndFunctions = { "+", "-", "*", "/", "^", "_", "sin", "cos", "(", ")" };
    while (index < sizeExp) {
        int oldIndex = index;
        ...
        if (isVariable(token)) {
            ...
            if (std::all_of(token.begin(), token.end(), [](const char& ch) { return (isdigit(ch) or ch == '.'); })) {
                continue;
            }
        ...
        throw std::invalid_argument("Error: Do not know -> " + token + "\n");
    }
}